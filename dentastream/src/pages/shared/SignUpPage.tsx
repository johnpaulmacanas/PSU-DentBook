...
/**
 * DEV sign-up page with a role picker so all three dashboards can be tested.
 * In production, doctor/admin accounts should be provisioned by clinic staff —
 * remove the role <select> (and default to 'patient') before going live.
 */
  ...
  if (done) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-2 px-4">
        <div className="w-full max-w-sm rounded-2xl border border-stroke bg-white p-8 text-center shadow-sm">
          <h1 className="text-xl font-bold text-dark">Account created</h1>
          <p className="mt-2 text-sm text-dark-5">
            If email confirmation is enabled on this project, confirm via the link
            we emailed you. Otherwise you can sign in now.
          </p>
          <button
            onClick={() => navigate('/login', { replace: true })}
            className="mt-5 w-full rounded-lg bg-primary py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary/90"
          >
            Go to Sign In
          </button>
        </div>
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-2 px-4 py-8">
      <div className="w-full max-w-sm rounded-2xl border border-stroke bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-dark-5">Join the DentaStream clinic portal</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-dark-5">
              Role <span className="text-dark-6">(dev only)</span>
            </label>
            <select
              value={role}
